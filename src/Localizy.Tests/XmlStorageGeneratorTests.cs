...
namespace Localizy.Tests
{
    public class XmlStorageGeneratorTests
    {
        ...
        public void xmlCreatorGenerate()
        {
            ...
            var result = xmlCreator.Generate(typeof(TestTranslations).Assembly, x => x == typeof(TestTranslations), new CultureInfo("en"));
        ...
        public void xmlCreatorGenerateWithHash()
        {
            var provider = new LocalizationProvider();
            var xmlCreator = new XmlStorageGenerator(provider);
            var result = xmlCreator.Generate(typeof(TestTranslations).Assembly, x => x == typeof(TestTranslations), new CultureInfo("en"), generateHash: true);
