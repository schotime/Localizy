...
namespace Localizy
{
    public static class ObjectFormatter
    {
        ...
        {
            var retVal = target;
            foreach (var sections in path.Split('.').Select(property => property.Replace("]", string.Empty).Split(new[]{'['}, StringSplitOptions.RemoveEmptyEntries)))
            {
        ...
        private static object GetPropertyValue(object target, string name, string indexList)
        {
            ...
            var property = target.GetType().GetProperty(name, null, indexes.Select(index => index.GetType()).ToArray());
            if (property == null)
